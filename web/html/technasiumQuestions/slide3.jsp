<%-- 
    Document   : slide3
    Created on : Dec 15, 2014, 1:50:59 PM
    Author     : mkslofstra
--%>

<div class="imageBoxes" id = "questionImage"> 
    <img data-imgNumber="3" id = "image"src="images/verhoorManager.jpg" alt=""/> 
    <div class = "left">
        <p class="triangle-border top">
            U weet dat *naam slachtoffer* door is aangetroffen in het lab waar u de manager van bent. U was op dat moment aanwezig 
            op het lab volgens administratie. Wat was u aan het doen op het moment van de moord?
        </p>
    </div>
    <div class ="right">
        <p class="triangle-border right">
           Ik was al die tijd bezig met het inplannen van nieuwe labroosters.
        </p>
    </div>
</div>

